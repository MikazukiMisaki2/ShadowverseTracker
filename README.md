...
当前迁移在 `ui/pyside6-migration` 分支。该分支将默认界面切换为 PySide6 + QFluentWidgets：左侧导航、卡片式仪表盘、牌组管理、Meta 卡组、概率计算、详细统计和设置分别独立成页，读取服务与训练记录仍复用现有后端。旧 Tk 界面保留在 `shadowverse_tracker.app`，便于回退和对照。
...
- “我的卡组”页支持导入牌组和完整浏览卡图；当前牌组会显示 SVWB 职业标志；已保存的本地牌组会自动加入“Meta 卡组”页（仅内存合并，不污染网站缓存），Meta 页面使用纯文字缓存展示主流构筑，避免打开页面时加载大量卡图。
- 默认开启本地胜负统计，提供总胜率、对手职业、先手/后手等数据；“详细统计”页面可按牌组查看和重置记录，也可以在“全部卡组”范围确认后一次重置所有卡组统计。
- 对局中会根据对手已经公开打出的卡牌，匹配离线缓存的 WBArts 主流构筑；记录和详细统计统一使用网站中文 archetype 名称，不显示构筑 A/B 或作者名，相同类型的多个具体构筑会归为同一类型，证据不足时保留为空。已有 `logs/app_session.jsonl` 的历史终局也会在启动后尝试回填，手动输入的标签不会被覆盖。
...
识别器和“Meta 卡组”页都使用 `src/shadowverse_tracker/data/meta_decks.json`（发布包中位于
`shadowverse_tracker/data/meta_decks.json`）中的具体 40 张牌构筑。Tracker 启动时会检查本地缓存日期；当天已经检查过就不会再次联网，超过一天才会在后台从
[`sva.hypd.asia/deck`](https://sva.hypd.asia/deck) 按网站职业筛选分别请求，每个职业取推荐排序前 3 套具体构筑。请求失败时保留旧缓存，不会阻塞主界面。缓存通常写入：

```text
%LOCALAPPDATA%\ShadowverseTracker\meta_decks.json
```

也可以在 WBArts 卡组详情页取得编号后，单独刷新缓存：

```powershell
python .\scripts\refresh_meta_decks.py 13337 150502
```

默认缓存来自 2026-09-05 的 WBArts 环境快照，包含 24 个主流 archetype、45 套具体构筑，并按网站的 T1、T2、T3、T4、其他分类展示；新一轮职业推荐刷新时每个职业最多加入 3 套，旧缓存不足时至少保留两套。刷新失败时（例如网络返回 Cloudflare 浏览器验证），原缓存不会被覆盖。对手隐藏手牌、尚未打出的牌和生成牌不能用于唯一识别，因此早期对局可能显示“未确认”；这时仍可双击详细统计里的“对手卡组”列手动填写。
