...
- 默认开启本地胜负统计，提供总胜率、对手职业、先手/后手等数据；“详细统计”页面可按牌组查看和重置记录。
- 对局中会根据对手已经公开打出的卡牌，匹配离线缓存的 WBArts 主流构筑；只有证据足够且候选差距明显时，才把具体构筑写入对局记录，无法确认时保留为空。
...
识别器不会在对局读取线程中实时抓取网页，而是使用 `src/shadowverse_tracker/data/meta_decks.json`（发布包中位于
`shadowverse_tracker/data/meta_decks.json`）中的具体 40 张牌构筑。当前缓存包含 WBArts 的两套“中速梦”示例构筑；可以在 WBArts 卡组详情页取得编号后，单独刷新缓存：

```powershell
python .\scripts\refresh_meta_decks.py 13337 150502
```

默认缓存来自 2026-09-05 的 WBArts 环境快照，包含 24 个主流 archetype、45 套具体构筑（有两个样本的 archetype 会标为 A/B；网站只返回一个样本的则保留一套）。刷新失败时（例如网络返回 Cloudflare 浏览器验证），原缓存不会被覆盖。对手隐藏手牌、尚未打出的牌和生成牌不能用于唯一识别，因此早期对局可能显示“未确认”，这是为了避免把错误构筑写进历史记录。
